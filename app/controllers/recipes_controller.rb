class RecipesController < ApplicationController
  ...
  def search
    ...
    if @course.blank?
      @url = "http://api.yummly.com/v1/api/recipes?_app_id="+yumid+"&_app_key="+yumkey+"&q=#{@search}&maxTotalTimeInSeconds=#{@seconds}"
    else
      @url = "http://api.yummly.com/v1/api/recipes?_app_id="+yumid+"&_app_key="+yumkey+"&q=#{@search}&allowedCourse[]=course^course-#{@course}&maxTotalTimeInSeconds=#{@seconds}"
    end
